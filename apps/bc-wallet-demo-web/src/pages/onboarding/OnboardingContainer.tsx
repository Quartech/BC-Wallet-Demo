...
export const OnboardingContainer: FC<Props> = ({
  ...
  const isForwardDisabled: boolean =
    !currentStep || //|| currentScenario?.steps.length === currentStep.order
    (!!currentStep?.actions?.some((action) => action.actionType === StepActionType.SetupConnection) &&
      !connectionCompleted) ||
    (!!currentStep?.actions?.some((action) => action.actionType === StepActionType.AcceptCredential) &&
      (!credentialsAccepted || credentialDefinitions.length === 0))
