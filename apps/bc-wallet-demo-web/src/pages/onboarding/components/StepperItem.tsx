      ...
      <div className="flex text-grey dark:text-white relative">
        <div
          className={`rounded-full transition duration-1000 ease-in-out py-3 h-12 w-12 border-2 ${
            currentStepIsEqual
              ? 'bg-bcgov-blue dark:bg-bcgov-black border-2 border-bcgov-blue dark:border-bcgov-gold text-white'
              : `${currentStepIsLower && currentStepIsNotEqual ? 'grayscale' : ''}`
          } ${
            currentStepIsHigher && currentStepIsNotEqual
              ? ' border-2 border-bcgov-blue dark:border-bcgov-gold bg-white dark:bg-bcgov-black text-bcgov-blue'
              : ''
          } `}
        >
          <div className={`flex items-center justify-center h-full dark:text-bcgov-gold`}>{step.order}</div>
        </div>
      </div>
      {step.order !== maxSteps && (
        <div
